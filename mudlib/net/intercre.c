...
mapping muds_intercre= ([
        "RL":({"147.156.10.98",3011,"a_password"}),
	"ILU":({"147.156.10.98",3010,"a_password"}),
        ]);
...
switch(servicio) {
	        ...
	        switch(canal) {
                        ...
                        respuesta=nice_list("/global/do_chat.c"->query_who_on(canal)->query_cap_name());
			transmitir(nombre_mud,"respuesta",usuario,"escuchando",respuesta);                                        break;
   	        ...
   	        if(sizeof(users())) break;
                ...
                break;
case "escribir":tell_object(find_living("vilat"),"tratando de escribir :"+msj+"\n");write_file("/open/"+canal,msj);break;
                default:
                switch(canal) {
                        case "chat":
...
if (servicio=="quien"|| servicio=="reboot" || servicio=="escuchando" ) {
