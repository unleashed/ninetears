...
void setup() {
  ...
  set_short("%^BOLD%^BLACK%^Minas de Keeroon: %^RESET%^Nivel 2");
   ...
   "estruendo viene del fondo de la mina. Y el aire comienza a ser mas irrespirable "
   "a cada paso que das.""\n\n");
...
add_item(({"filon","filones","carbon","hierro","cobre"}),"Son varios filones de distintos tipos, muy pequenyos, pero se  nota que son de hierro, cobre y carbon.\n");

add_clone(NPCSMINAS+"bixos_minas2.c",random(4));

//Salidas disponibles
add_exit("norte","d/keroon/minas/minas2/minas2_121.c","standard");
add_exit("sur","d/keroon/minas/minas2/minas2_152.c","standard");
add_exit("noreste","d/keroon/minas/minas2/minas2_122.c","standard");
}
