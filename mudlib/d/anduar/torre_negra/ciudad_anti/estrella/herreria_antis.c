// Herreria para la Ciudad de Antipaladines
...
{ 
   if(!herrero)
   {
      herrero=clone_object(NPCS+"herrero_antis");
...
void setup()
{
  set_short("%^BOLD%^%^BLACK%^Herreria de Gidecca%^RESET%^");
  ...
  "  <%^MAGENTA%^examinar%^RESET%^> para examinar un objeto y ver cuanto vale\n\n");
   ...
   set_light(60);
   add_exit("fuera",CIUDAD"ci_6", "door");
   set_store_room("/d/anduar/anduar/ciudad/almacen4.c");
