...
// Para que no sigile en pelea:
atts_list = TP->query_attacker_list();

//tell_object(find_player("ather"),"numero peleas: "+sizeof(atts_list)+"\n");

for (j=0;j<sizeof(atts_list);j++)
{
      if(ENV(atts_list[j])==ENV(TO))
      {
      sinsigilar=1;
      }
}     
...
{
sinsigilar=0;
tell_room(ENV(TO),TO->query_short()+" se va a "+ultimasalida+".\n");        
this_object()->move(dest_dir[d]);      
tell_room(ENV(TO),TO->query_short()+" llega del "+ultimasalida+".\n");        
}
else
{
this_object()->move(dest_dir[d]);      
