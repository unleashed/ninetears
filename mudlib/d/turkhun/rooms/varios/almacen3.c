...
setup()
{
	...
	add_exit("south", VARIOS+"panaderia", "door");
	set_light(80);
}
...
void af_reset()
{
   ...
   for (;i<3;i++)
   {
     ob=clone_object(ITEMS+"tarta_fresa")->move(this_object());
   }
   i = sizeof(find_match("rons", this_object()));
   for (;i<4;i++)
   {
     ob=clone_object(ITEMS+"tarta_ron")->move(this_object());
   }
   i=sizeof(find_match("panes de viaje", this_object()));
   for (;i<20+random(10);i++)
   {
      ob=new(ITEMS+"barra_cuarto")->move(this_object());
   }
   i=sizeof(find_match("grandes panes", this_object()));
   for (;i<18+random(8);i++)
   {
      ob=new(ITEMS+"barra_medio")->move(this_object());
   }
}
