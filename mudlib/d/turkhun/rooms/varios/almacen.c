...
setup()
{
        ...
        add_exit("south", CIUDAD+"tienda", "door");
        set_light(80);
}
...
void af_reset()
{
   ...
   i=sizeof(find_match("fracon cristalino",TO));
/*   for (;i<1;i++)
   {
      ob=new("/d/golthur/items/subnivel5/frasco.c");
      ob->move(this_object());
   }

   i=sizeof(find_match("bolsa",TO));
   for (;i<1;i++)
   {
      ob=new("/d/anduar/objetos/bolsa.c");
      ob->move(this_object());
   }
*/
}
