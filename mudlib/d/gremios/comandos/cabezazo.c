...
int condiciones() {
  if (me->query_dead()) return notify_fail("Eres un espiritu, no serviria de mucho embestir a alguien.\n");
  ::condiciones();
  if (!ob||ob->query_dead()) return notify_fail("Tu objetivo ha muerto y no puedes embestirle.\n");
  if (ENV(ob)!=ENV(me)) return notify_fail("Tu oponente ha huido rapidamente!!\n");
  if ( me->query_ep() < 0 ) return notify_fail("No tienes suficiente energia para darle un cabezazo a alguien.\n");
  return 1;
}
...
int efecto( string inutil, object masinutil ) {
  ...
  mychance=me->query_level()+me->query_total_ac()+20+me->query_habilidad("cabezazo");
