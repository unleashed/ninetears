...
{
  tell_object(my_player, "La Marca Oscura de tu piel desaparece.\nSuspiras aliviado.\n");
  tell_room(environment(my_player),my_player->query_cap_name()+
    "suspira aliviado cuando su Marca Oscura desaparece.\n", my_player);
...
int check_duration()
{
  if ( !(my_player->query_timed_property("marca_on")) )
    return 1;
  else return 0;
}
...
void init()
{
  ...
  my_amount = this_object()->query_property("marca damage");
}
...
void set_spell(object ob,int amount)
{
  ...
  my_count = 0;
  my_amount = amount;
  add_static_property("marca damage", amount);
...
void heart_beat()
{
  ...
  if ( !my_amount )
    my_amount = (int)this_object()->query_property("marca damage");
...
string extra_look()
{
  return "Tiene una horrible %^BLACK%^BOLD%^Marca Oscura %^RESET%^en la piel.\n";
}
