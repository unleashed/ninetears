...
int round1(object caster, mixed target, mixed out_range, int time, int quiet)
{
  ...
  else
  {
    ...
    tell_object(caster,"Silencias a "+target->QCN+", y "+target->query_pronoun()+" te grita silenciosamente.\n");
    tell_object(target,caster->QCN+" hace unos gestos sobre tu cuello y te quedas sin habla!!!\n");
  }
