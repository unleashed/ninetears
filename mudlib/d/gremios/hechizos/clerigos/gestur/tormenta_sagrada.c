...
int	NIVEL	= 7;
mixed	ESCUELA	= ({""});
mixed	ESFERA	= ({"combate"});
string	CANCION	= "'deus ire'";
string	TIPO	= "magical";
int	SALV	= 100;
int	SIZE_D	= 6;
mixed	BADGUYS	= ({"Cleriga de Sinister"});
...
int round1(object caster, mixed target, mixed out_range, int time, int quiet)
{
  ...
  for (i=0;i<sizeof(target);i++)
  {
    ...
    if (j<100 )
    {
      ...
      tell_object(target[i],"Una tormenta de simbolos sagrados se cierne sobre ti.\n");
  ...
  tell_room(ENV(caster),caster->QCN+" envuelve a "+str+" en una tormenta de simbolos sagrados!\n",({caster,target}));
  tell_object(caster, "Envuelves a "+str+" en una tormenta de simbolos sagrados!\n");
  return 0;
}
