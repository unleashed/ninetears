...
#define NOMBRE  "Enjambre de meteoros sombra"
#define ESCUELA ({"ilusion"})
#define	ESFERA	({""})
#define CANCION "'shadowyz meseious pyros blasious'"
#define NIVEL	9
#define TIPO	"tierra"
#define	SALV	50
#define SIZE_D	4
...
int round2(object caster, mixed target, mixed out_range, int time, int quiet)
{
  ...
  for (i=0;i<sizeof(target);i++)
  {
    ...
    {
      DETECTED+=caster->QL+POWER-target[i]->query_int();
      DETECTED=DETECTED<10;
    }
    if (DETECTED)
    {
    tell_object(caster,target[i]->QCN+" detecta tu ilusion!\n");
    tell_object(target[i],"Descubres que el hechizo de "+caster->QCN+" es una ilusion.\n");
    } else {
