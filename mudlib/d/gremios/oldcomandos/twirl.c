...
int twirl(string str, object doer)
{
  int skill,s1,i;
  int j,x;
  int damage;
...
*/
	...
	if (me->query_hidden())
		tell_room(ENV(me), "Escuchas el sonido del acero cortando el aire y cogiendo una temible aceleracion...\n", me);
	else
...
int begin_twirl(mixed *params)
{
	...
	if (ENV(params[1]) != ENV(params[5])) {
		notify_fail("Tu victima ha huido cobardemente al sentir tu acero de cerca.\n");
		if (params[5]->query_hidden())
		tell_room(params[5], "De pronto el ruido de acero cortando el aire se extingue.\n");
		else
		tell_room(params[5], "Las espadas de " + params[5]->QCN + " dejan de cortar el aire.\n");
		return 0;
	}
	tell_room(ENV(params[5]),
                params[5]->QCN + " entierra sus cuchillas en " + params[1]->QCN + " a una vertiginosa velocidad!\n",
		({params[5],params[1]}));
	tell_object(params[1], params[5]->QCN + " entierra sus armas en ti moviendo sus brazos endiabladamente!\n");
	tell_object(params[5], "Entierras tus armas en "+params[1]->QCN+" moviendo tus brazos endiabladamente!\n");
