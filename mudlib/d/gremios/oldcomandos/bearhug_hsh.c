				...
				followee, mixed enter)
{
  /*tell_object(my_player, "You cannot move while you are bearhugging "
    "someone!\n");*/
  notify_fail("You cannot move while you are bearhugging "
    "someone!\n");
  if ( check_duration() )
...
{
  tell_object(my_player, "Aprietas con fuerza a "+my_target->query_cap_name()+
    " y "+my_target->query_pronoun()+" sufre agonicamente.\n");
  tell_room(environment(my_player), my_player->query_cap_name()+" aprieta "
	"sus brazos sobre "+my_target->query_cap_name()+".\n",
    ({ my_player, my_target }) );
  tell_object(my_target, my_player->query_cap_name()+" aprieta sus "+
    "brazos contra tu cuerpo haciendote gritar agonicamente.\n");

  my_target->adjust_hp(-(random((int)my_player->query_damage_bonus())+random(14)+1), my_player);
