...
int charge(string str,object cast)
{
  ...
  caster->add_timed_property("nocast",1,8);
...
mixed spell(string str, object caster)
{
  ...
  if ( ob == caster )
    return "Milord!  Suicidarse no entra en sus planes por el momento!\n";
...
int hb_spell(object me, mixed *params, int time)
{
  ...
  switch( total_time - time )
  {
   ...
   {
	tell_object(me,"Huya del campo de batalla como el cobarde que usted es!.\n");
	   me->add_timed_property("noguild",1,5);
	/* mmm penalizar sin comandos durante este time? hmmmmmmmmmmmmm... */	
	   call_out("end_charge",0,me);
		return 1;
   }
	...
	if(!him)
	{
	  tell_object(me, "Su objetivo ha perecido antes de poder cargar contra el.\n");
	call_out("end_charge",0,me);
	return 1;
	}
	if ( environment(me) != where) // esto rula si se va antes el palata ke el enemigo
	{
		tell_object(me, "Deshonroso acto, no es usted digno de su gremio.\n");
		tell_object(him, me->query_short()+" se ha ido con el rabo entre las piernas... dale su merecido!\n");
		tell_room(where, "Deshonrosamente, "+me->query_short()+" corre por su vida.\n",
			({me, him}));
		me->add_timed_property("noguild", 1, 5);
		call_out("end_charge",0,me);
		return 1;
	...
	if ( environment(me) != environment(him))
      {
        ...
        tell_object(him, me->query_cap_name()+" viene chillando hacia "
          "ti y carga contra ti mientras escapas!\n");
      }  
	...
	return 1;
      ...
      {  
        ...
        tell_object(him, me->query_cap_name()+" baja su "
          "arma pausadamente, golpeandose en "
          "la cabeza.\n");
