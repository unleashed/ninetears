...
// permanently instead of static.
   ...
   if(doer->query_gp() < START_GP){
      tell_object(doer,"Estas demasiado exhausto.\n");
      return 1;
   }
   ...
   if(point == 2){
      tell_object(doer,"Estas enrabiado! no puedes volver a "
       "enrabiarte.\n");
      return 1;
   }
...
int hb_command(object doer, int time, mixed *params){
   ...
   if(doer->query_property("flipped") == 1){
      ...
      if(!random(2)){
         tell_object(doer,"Te enrabias contra tu oponente.\n");
         tell_room(environment(doer),doer->query_cap_name()+
          " se enrabia contra "
          "su oponente.\n",doer);
      }
      if(random(5000) < chance){
         tell_object(doer,"Sientes como pierdes el control!!!\n\n");
   ...
   if(doer->query_property("flipped") == 2){
      ...
      if(!random(2)){
         tell_object(doer,"Atacas todo lo que hay a tu alrededor con rabia.\n");
         tell_room(environment(doer),doer->query_cap_name()+
          " esta enrabiado atacando a todo el mundo!\n",doer);
      }
      foreach(co in cows){
// grrr hack hack hack
         if(!co) continue;
...
void property_check(object doer){
   ...
   if(doer->query_timed_property("feared")){
      ...
      if(doer->query_property("flipped") == 1 &&
         ...
         doer->set_wimpy(0);
         tell_object(doer,"La colera corre por tus venas, "
          "comienzas a llenarte de una terrible rabia.\n");
...
void remove_me(object doer){
   ...
   if(bstats[0]){
      ...
      tell_room(environment(doer),doer->query_cap_name()+
       " se desploma y sus emociones "
       "salen de su cuerpo.\n",doer);
      tell_object(doer,"Te sientes exhausto y tu colera se desvanece.\n");
      bstats = 0;
