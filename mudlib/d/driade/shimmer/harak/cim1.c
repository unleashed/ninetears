...
void setup() {
   ...
   set_zone("ladera");
   set_light(40);
   set_short("%^WHITE%^Cerca de la Cima del %^BOLD%^%^BLACK%^Harak-Din%^RESET%^");
   set_long("%^WHITE%^Cerca de la Cima del %^BOLD%^%^BLACK%^Harak-Din%^RESET%^\n\n"
...
add_clone(NPCS+"varios_cima_harak.c",1);

add_exit("noreste",	HARAK+"cim2.c","standard");
add_exit("abajo",   HARAK+"lad52.c","standard");
}