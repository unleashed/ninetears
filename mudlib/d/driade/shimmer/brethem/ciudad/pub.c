...
void create() {
  ...
  set_short("Pub");
  set_long(
    "Estas en una Taberna.  Aqui puedes tomarte bebidas y algunas tapas.\n"
    "Hay un menu ahi que puedes leer.\n");
  set_light(60);
  menu_header = "El menu dice:";
  display_headings = 1;
  ::create();
...
int do_read(string str)
{
  ...
  if((temp = do_race_adjust(temp,this_player()->query_race()))==
         -101) {
                write("Este establacimiento no quiere nada con "
                      "los de tu raza.\n");
                return 1;
  }
      write(read());
...
// I hope cost didn't need to be mixed :)   Anirudh
  ...
  if(forplay > 0)
  {
      ...
      {
        notify_fail("Comprar lo que por cuanto ?.\n");
        return 0;
      }
      notify_fail("Mmm, esa persona no parece estar aqui.\n");
   forwho = present(list[forplay+1]);
      ...
      {
        write((string)forwho->query_cap_name()+" rechaza tu invitacion.\n");
        tell_object(forwho,this_player()->query_cap_name()+" intenta invitarte a algo, pero tu lo rechazas.\n");
  ...
  if(!menu_items[str]) 
   {
    notify_fail("Perdona, "+str+" no esta en el menu.\n");
    return 0;
  }
  ...
  if((cost = do_race_adjust(cost,this_player()->query_race()))==
          -101) { notify_fail("Este establecimiento no quiere "
                     "tratos con los de tu raza.\n");
                  return 0;
  }
  if(this_player()->query_value() < cost) {
    notify_fail("Eres demasiado pobre para comprar eso.\n");
    return 0;
  }
...
// Cailet 1997 for songster quest
  ...
  if(error) {
    write("No puedes llevar mas de eso.\n");
    food->dest_me();
    return 1;
  }
  this_player()->pay_money( ({"copper",cost }) );
