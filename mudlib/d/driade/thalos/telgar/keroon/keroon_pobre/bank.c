...
int get_account() 
  ...
  if (!undefinedp(this_player()->query_property("banco "+save_file)))
    return (int)this_player()->query_property("banco "+save_file);
...
int set_account(int amt) 
  ...
  this_player()->add_property("banco "+save_file, amt);
} /* set_account() */
...
if(this_player()->query_dead())
{
  ...
  say(this_player()->query_cap_name()+" recupera dinero de su "+
      this_player()->query_possessive()+" cuenta.\n");
  return 1;
} /* withdraw() */
...
/* Added by Timion, 06 NOV 97
  ...
  write(capitalize((string)MONEY_HAND->money_value_string(total_amt))+
        ...
        ".\n");
  say(this_player()->query_cap_name()+" deposita algun dinero en la cuenta de "+
