...
int round1(object caster, mixed target, mixed out_range, int time, int quiet)
{
  ...
  {
    tell_object(caster,"El fantasma no desea volver a la vida por el momento. (ver 'ayuda consentir')\n");
    return -1;
  }
  ...
  if((caster->query_guild_name() == "Clerigo de Yvendur" && target->query_race_name() != "Drow") && 
    random(100)>5 && caster->query_level()>12)
  {
