...
#define	ESFERA	({"paladin"})
#define	CANCION	""
#define	NIVEL	6
#define	TIPO	"curacion"
#define	SALV	100
...
int round3(object caster, mixed target, mixed out_range, int time, int quiet )
{
  ...
  damage=fix_damage(caster,target,target->query_max_hp()-target->query_hp());
	if (caster->query_creator())
		printf("Curacion de %d pvs.\n", damage);
 ...
 */
 // para que no se sanen en seguida
  caster->remove_timed_property(file_name(TO));
  caster->add_timed_property(file_name(TO), 1, 40);
  return NIVEL*100;
}
