...
void find_tracks()
{
   ...
   if(hunted && environment(hunted))
   {
      if(environment(hunted) == environment(me))
      {
         ...
         // Renew the shadows...
	// pufff, la de bugs de shadows puestas por unos y otros.
	// mucho work para evitar shadow sobre shadow y asi
         hunted->destruct_tracked_shadow();
         me->destruct_tracker_shadow();
         "/d/gremios/comandos/track.c"->load_shadows(me, hunted);
         return;
      }
   }
   dir = environment(me)->query_timed_property("track_"+victim);
   if(dir)
      tell_object(me,"Las huellas de "+victim+" conducen hacia "+dir+".\n");
   return;
}
...
void clean_tracking()
{
   ...
   call_out("destruct_tracker_shadow",0);
   if(me)
      tell_object(me,"Dejas de rastrear a "+victim+".\n");
   return;
}
