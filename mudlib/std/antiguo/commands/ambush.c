// HAY QUE REMODELAR A SACO, AHORA MISMO DEJA EMBOSCAR A LA VISTA Y
// DIRECTAMENTE, SIN ESPERAR NADA, COMO UNA PUNYALADA
...
int ambush(string str,object x) 
{
    ...
    if( (int)victim->query_property("sens_life"))
    {
	...
	// no hace falta quitar la shadow de hide aki, ia se kita al
	// hacer el danyo
	//if(me->query_hide_shadow())
	//   me->dest_hide_shadow();
	aweap->event_ambush(damage,me,victim);
	me->add_timed_property("guildsuc",1,2); // ???
	tell_room(ENV(victim), me->QCN + " golpea brutalmente a " +
		victim->QCN + " en la cabeza !!\n", ({ me, victim }));
...
// esta parte no rulaba bien escondido, me hizo arreglar el bug del
// call_out de la shadow de esconderse. Ahora se entra siempre en peleas
// ya que antes el attack_by de living/combat.c preguntaba si el agresor
// estaba escondido y como la shadow aun no se habia destruido (por el
// call_out) no hacia efectivo que el agresor entrara en peleas
// con la victima si aquel se iba (mas bien, el agresor no veia lo de
// Estas persiguiendo a victima..., aunque la victima si pq no tenia
// shadow!)
    victim->attack_by(me);
// siiii weno, osea! xD
    me->attack_by(victim);
