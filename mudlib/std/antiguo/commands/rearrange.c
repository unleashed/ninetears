// ESTE ES EL USADO /table/command_table.c
...
int intro() {
  ...
  tell_object(this_player(),"Fuckin' Fic Mud now uses a rolling "
...
void confirm_order(string answer) {
 ...
 if(member_array(answer,({"y","Y","yes","n","N","no","s", "S", "si"})) == -1) {
   tell_object(this_player(),"You must answer y or n:  ");
   input_to("confirm_order",0);
   return;
 }
...
void finish_rolling(string answer) {
  ...
  if(member_array(answer,({"s", "S", "si", "y","Y","n","N","yes","no"})) == -1) {
    tell_object(this_player(),"Answer y or n:  ");
    input_to("finish_rolling",0);
    return;
  }
  ...
  if(member_array(answer,({"y","Y","yes","s","si","S"})) != -1) {
...
void confirm_exit(string answer) {
  ...
  if(member_array(answer,({"y","Y","yes","s","S","si"})) != -1) {
...
void store_values() {
  ...
  log_file("stat.rolls",ctime(time())+": "+this_player()->query_cap_name()+
	...
	/* HACEMOS LA TRAMPA, NO SE PQ SE DOBLAN LOS BONOS */
	scores[0] = 18 + this_player()->query_bonus_str() / 2;
	scores[1] = 18 + this_player()->query_bonus_dex() / 2;
	scores[2] = 18 + this_player()->query_bonus_con() / 2;
	scores[3] = 18 + this_player()->query_bonus_int() / 2;
	scores[4] = 18 + this_player()->query_bonus_wis() / 2;
	scores[5] = 18 + this_player()->query_bonus_cha() / 2;
...
// leviathan
// en el original, query_lord
    if(ppl[i]->query_creator() && ppl[i]->query_property("roller_inform"))
      tell_object(ppl[i],"Ghost [Info] WARNING!  "+person+" ha tirado "
                  +times+" veces en los ultimos "+howlong+
                  " minutos.\n");
  return;
}
