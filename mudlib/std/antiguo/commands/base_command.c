...
/* was a syntax check in here before, not needed now I hope */
   ...
   if(doer->query_property("comm_lockout")||
     doer->query_property(sec_lock_prop)){
      ...
      tell_object(doer,"Todavia no te encuentras en condiciones de hacerlo.\n");
      return 1;
   }
   if(doer->query_property("noguild") ||
    environment(doer)->query_property("noguild") ||
    doer->query_property("passed out")){
      tell_object(doer,"No puedes hacer en este momento.\n");
      return 1;
   }
   ...
   if(!sizeof(doer->query_attacker_list()) && (flags & 32)){
      mess = "No tienes ningun oponente delante.\n";
      target = ({});
   }

   if(sizeof(doer->query_attacker_list()) && (flags & 64)){
      mess = "Ahora mismo estas luchando.\n";
      target = ({});
   }
   ...
   for(i=0;i<sizeof(target);i++){
      ...
      if(!(flags & 4)){
         ...
         if(doer->query_property("pacify_on")){
            if(prior < 2){
               mess = "Te sientes demasiado pacifico para hacer algo al respecto.\n";
               prior = 2;
            }
            target[i] = 0;
            continue;
      ...
      if(!doer->query_creator() && target[i]->query_creator() &&
         ...
         if(prior < 2){
            mess = "Como te atreves a intentar esto con el inmortal "
   ...
   if(strlen(mess)){
      ...
      tell_object(doer,""
       +((flags & 16)?"Nada":"Nadie")+" aqui con ese nombre.\n");
      doer->adjust_gp(-1);
      return 1;
   }
