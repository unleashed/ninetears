...
int intro() {
  ...
  tell_object(this_player(),"\n\nCalandria Mud usa un metodo de rol "
              "para generar los atributos del player.\nEstos atributos "
              "son:\n"
              "STRength (fuerza), DEXterity (dextreza), CONstitution (constitucion),\n"
              "INTelligence (Inteligencia), WISdom (sabiduria),y CHArisma (carisma).\n"
              "La relevancia de cada atributo depende de que clase de player que quieras.\n"
              ...
              "escribe el orden en el que tu valoras tus atributos ( de "
              "mas valor a menos ).\nEjemplo: str dex con "
              "int wis cha\nlos Scripts y otra forma de automatizacion estan "
              "estrictamente PROHIBIDOS en este proceso.\n"
              "Usa 'quit' para salir.\n\nOrden>  ");
...
void do_the_roll() {
  ...
  tell_object(this_player(),"Tus tiradas son:\n\t"+thestats[statorder[0]]+
...
void finish_rolling(string answer) {
  ...
  tiradas=TP->query_property("tiradas_b");
  TP->add_property("tiradas_b",tiradas+1);

  tell_object(this_player(),"Tiradas borradas.\n");

  if(tiradas>100)
     if(tiradas>(TP->query_property("rolltime")))
        { TP->remove_property("tiradas_b");
          TP->remove_property("rolltime");
          TP->remove_property("timesrolled");
          TP->remove_property("roll_status");
          log_file("autoroll",ctime(time())+": "+TP->query_cap_name()+" ha sido expulsado por ajustar "
                   "automaticamente.\n");
          tell_object(TP,"No puedes usar metodos de automatizacion para tus tiradas, lo siento ...\n");
          call_out("dest_me",0);
          TP->quit();
        } 
