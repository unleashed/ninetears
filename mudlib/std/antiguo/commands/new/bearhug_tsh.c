 ...
 */
  ...
  if ( t_roll > h_roll )
  {
    tell_object(my_player,
      my_hugger->query_cap_name()+" te liberas del abrazo del oso!\n");
    tell_room(environment(my_player), my_player->query_cap_name()+
      " hace fuerza para liberarse de los brazos de "+my_hugger->query_cap_name()+" y lo consigue!\n",
      ({ my_hugger, my_player}) );
    tell_object(my_hugger, my_player->query_cap_name()+
      " se libera del abrazo del oso!\n");
    my_player->remove_static_property("noguild");
...
{
  tell_object(my_player, "No puedes escapar del abrazo de "+my_hugger->query_cap_name()+
    "!\n");
  return "";
}
...
{
  tell_object(my_player, "Haces fuerza para escapar del abrazo de "+
    my_hugger->query_cap_name()+".\n");
  tell_room(environment(my_player), my_player->query_cap_name()+" hace fuerza para"
    " escaparse de "+my_hugger->query_cap_name()+".\n",
    ({ me, him }) );
  tell_object(my_hugger, "Estrangulas a "+my_player->query_cap_name()+" con  "
    "tu abrazo.\n");
  if ( check_duration() )
...
attack_ob(object ob) 
{
  tell_object(my_player, "No estas en posicion de atacar a alguien.\n");
  return 0;
}
...
int cast()
{
  tell_object(my_player, "Estas en mala posicion para hacer esto.\n");
  return 1;
...
int adjust_hp(int amount, object doer)
{
  ...
  if ( random(100) > thres )
  {
    tell_object(my_hugger, "Te haces un rasgunyo por el forcejeo!\n");
    tell_room(environment(my_player), my_hugger->query_cap_name()+
      " se hace un rasgunyo en el forcejeo!\n",
      my_hugger);
