...
inherit "/d/gremios/comandos/target.c";
...
/* needless to say. having more than one of 'on, in or from' is bad */
      if(strsrch(str," en ") >= 0) // it's an item in someone/something
         sscanf(str,"%s en %s",str1,str2);
      if(strsrch(str," en ") >= 0)
         sscanf(str,"%s en %s",str1,str2);
      if(strsrch(str," del ") >= 0)
         sscanf(str,"%s del %s",str1,str2);
   }
...
int exec_command(string str, object doer){
   ...
   if(!env){
      tell_object(doer,"No puedes hacer esto aqui y ahora.\n");
      return 1;
   }
...
/* was a syntax check in here before, not needed now I hope */
   ...
   if(!sizeof(doer->query_attacker_list()) && (flags & 32)){
      mess = "Tu oponente no esta en frente de ti.\n";
      target = ({});
   }

   if(sizeof(doer->query_attacker_list()) && (flags & 64)){
      mess = "Estas luchando ahora.\n";
      target = ({});
   }
   ...
   for(i=0;i<sizeof(target);i++){
      ...
      if(target[i]->query_dead() && interactive(target[i]) && !(flags & 2)){
         if(prior < 1){
            mess = target[i]->query_cap_name()+" esta muerto.\n";
            prior = 1;
         }
         target[i] = 0;
         continue;
      ...
      if(!(flags & 4)){
         if(target[i]->query_property("pacify_on")){
            if(prior < 1){
               mess = target[i]->query_cap_name()+" es inofensivo.\n";
               prior = 1;
            }
            target[i] = 0;
            continue;
         ...
         if(doer->query_property("pacify_on")){
            if(prior < 2){
               mess = "Sientes como la paz te envuelva por todas partes.\n";
               prior = 2;
            }
            target[i] = 0;
            continue;
      ...
      if(!doer->query_creator() && target[i]->query_creator() &&
         ...
         if(prior < 2){
            mess = "Como osas intentar esto con un inmortal? "
      ...
      if(!living(target[i]) && !(flags & 16)){
         if(prior < 1){
            mess = "Intenta encontrar algo vivo.\n";
            prior = 1;
         }
         target[i] = 0;
         continue;
   ...
   if(strlen(mess)){
      ...
      tell_object(doer,""
       +((flags & 16)?"Nada ":"Nada ")+"hay aqui con ese nombre.\n");
      doer->adjust_gp(-1);
      return 1;
   }
