...
/* was a syntax check in here before, not needed now I hope */
   ...
   if(!sizeof(doer->query_attacker_list()) && (flags & 32)){
      mess = "No tienes a tu oponente delante de ti.\n";
      target = ({});
   }

   if(sizeof(doer->query_attacker_list()) && (flags & 64)){
      mess = "Estas en una lucha ahora.\n";
      target = ({});
   }
   ...
   for(i=0;i<sizeof(target);i++){
      ...
      if(!(flags & 4)){
         ...
         if(doer->query_property("pacify_on")){
            if(prior < 2){
               mess = "Te sientes demasiado pacifico para hacer algo a alguien.\n";
               prior = 2;
            }
            target[i] = 0;
            continue;
      ...
      if(!doer->query_creator() && target[i]->query_creator() &&
         ...
         if(prior < 2){
            mess = "Ni se te ocurra intentar esto con un inmortal !!"
   ...
   if(strlen(mess)){
      ...
      tell_object(doer,""
       +((flags & 16)?"Nada ":"Nada ")+"aqui con ese nombre.\n");
      doer->adjust_gp(-1);
      return 1;
   }
