...
int check_duration()
{
	...
	tell_object(my_player,
	  "Estas medio atontado y no puedes moverte mas rapido.\n");

	tell_room(environment(my_player), my_player->query_cap_name()+
	  " no parece moverse muy rapido.\n", my_player);
...
void dispell_me()
{
    ...
    tell_object(my_player,
      "Te mueves lentamente de nuevo consiguiendo volver a "
      "tu estado normal.\n");
    tell_room(environment(my_player), my_player->query_cap_name()+
      " se mueve normal de nuevo.\n",
      my_player);
