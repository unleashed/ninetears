...
inherit "/std/room";
#include "path.h"
#define FILE_SIZE 100000    // change, you die
      ...
      for(i=0; i<sizeof(all); i++)
      {
         if((int)all[i]->query_auto_load())
         {
            write("Depositas "+all[i]->query_short()+" "
               "en el baul, y se convierte en una nube de humno.\n\n");
            vault->dest_me();
            all[i]->dest_me();
            return(1);
         }
...
//         {
            ...
            write_file(vault_log,this_player()->query_short()+
               " deposito:  "+all[i]->query_short()+" a las "+
               ctime(time())+"\n");
//         }
      }
      vault->dest_me();
...
/*
   ...
   {
      notify_fail("No estaras intentado robar a %^BOLD%^RED%^Leviathan %^RESET%^verdad?\n\nSe lo notificare de todas formas... \n");
      write_file(vault_log,this_player()->query_short()+" intento %^BOLD%^RED%^ROBAR%^RESET%^:  "+all[i]->query_short()+
      " a las "+ctime(time())+"\n");
   ...
   if(sizeof(all))
   {
      for(i=0; i<sizeof(all); i++)
      {
            ...
            this_player()->query_max_weight())
         {
            ...
            write_file(vault_log,this_player()->query_short()+
               " recupero:  "+all[i]->query_short()+" a las "+
               ctime(time())+"\n");
         }
         else
