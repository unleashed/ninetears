...
void setup()
{
  set_short("Gremio de %^RED%^Guerreros Drow%^RESET%^");
  set_long("Gremio de %^RED%^Guerreros Drow%^RESET%^\n\n"
...
//  add_exit("north", ROOM+"stair2.c", "corridor");
  add_exit("oeste", "/room/caverna/cav12", "door");
//  modify_exit("south", ({"function", "go_daggfight"}));
  modify_exit("oeste", ({"function", "go_daggfight"}));
  set_guild("guerreros/slicerdrow");
	set_teaching_person("Turgon");
}

void reset()
{
	if (!guildmaster) {
		guildmaster = clone_object("/baseobs/guildmasters/turgon.c");
...
int go_daggfight()
{
  if(this_player()->query_guild_name() == "guerrero drow" && guildmaster)
  {
	if (!guildmaster->query_fighting())
		tell_object(this_player(), "Turgon exclama: Sirva tu espada para tu causa!\n");
  }
  return 1;
}
