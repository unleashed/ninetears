...
void setup()
{
  set_short("%^ORANGE%^Gremio de Magos Guerreros: Sala de Aprendizaje%^RESET%^");
  set_long("%^ORANGE%^Gremio de Magos Guerreros: Sala de Aprendizaje%^RESET%^\n\n"
...
//  add_exit("north", ROOM+"stair2.c", "corridor");
  add_exit("oeste", "/room/test_area/r02", "door");
  set_guild("misc/wizwar");
set_teaching_person("metzen");
}

void reset()
{
	if (!guildmaster) {
		guildmaster = clone_object("/baseobs/guildmasters/metzen.c");
