...
void setup()
{
  set_short("Gremio de Clerigos de Gestur: Sala de Aprendizaje");
  set_long("Gremio de Clerigos de Gestur: Sala de Aprendizaje\n\n"
...
//  add_exit("north", ROOM+"stair2.c", "corridor");
  add_exit("noreste", "/room/test_area/r01", "door");
  set_guild("clerigos/gestur");
set_teaching_person("laetitia");
}

void reset()
{
	if (!guildmaster) {
		guildmaster = clone_object("/baseobs/guildmasters/laetitia.c");
