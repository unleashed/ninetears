...
void setup()
{
  set_short("Gremio de %^RED%^Asesinos%^RESET%^: Entrada");
  set_long("Gremio de %^RED%^Asesinos%^RESET%^: Entrada\n\n"
...
//  add_exit("north", ROOM+"stair2.c", "corridor");
  add_exit("este", "/room/camino/caminito01", "door");
  set_guild("picaros/asesino");
set_teaching_person("Slackess");
}

void reset()
{
	if (!guildmaster) {
		guildmaster = clone_object("/baseobs/guildmasters/slackess.c");
