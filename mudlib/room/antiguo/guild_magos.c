...
void setup()
{
  set_short("%^BLUE%^Torre de Magos: Sala de Aprendizaje%^RESET%^");
  set_long("%^BLUE%^Torre de Magos: Sala de Aprendizaje%^RESET%^\n\n"
...
//  add_exit("north", ROOM+"stair2.c", "corridor");
  add_exit("abajo", "/room/torre_magos/entrada", "door");
  set_guild("magos/magobueno");

set_teaching_person("aenima");
}

void reset()
{
	if (!guildmaster) {
		guildmaster = clone_object("/baseobs/guildmasters/aenima.c");
