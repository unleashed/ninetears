...
void setup()
{
  set_short("%^RED%^Gremio de Barbaros: Area de Entrenamiento%^RESET%^");
  set_long("%^RED%^Gremio de Barbaros: Area de Entrenamiento%^RESET%^\n\n"
...
//  add_exit("north", ROOM+"stair2.c", "corridor");
  add_exit("sur", "/room/camino_avalon/camino11", "door");
//  modify_exit("south", ({"function", "go_daggfight"}));
//  modify_exit("norte", ({"function", "go_daggfight"}));
  set_guild("guerreros/barbaro");
	set_teaching_person("vrolgar");
}

void reset()
{
	if (!guildmaster) {
		guildmaster = clone_object("/baseobs/guildmasters/vrolgar.c");
