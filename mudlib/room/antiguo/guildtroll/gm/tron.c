...
void setup() {
	...
	set_gp(1000);
      ...
      set_long("Tron, el guardian del gremio de los Trolls.\n\n"
...
//      set_aggressive(0);
	...
	this_object()->init_equip();
      load_chat(20,
         ({
               1, "'Para ser un buen troll debes ser capaz de batirme en duelo!",
...
//               1, "@grin evilly $lname$",
	...
	set_guild("warriors/guildtroll");
	set_guild_ob("/d/gremios/guerreros/guildtroll");
...
int do_bash(string str) {
	...
	if (member_array("bash", TP->query_known_commands()) != -1) {
		notify_fail("Tron te dice: No puedo ensenyarte lo que ya sabes.\n");
		return 0;
	}
	if (TP->query_guild_name() != TO->query_guild_name()) {
		notify_fail("Tron te ensenya el arte del Bash.\n");
		return 0;
	}
	tell_object(TP, "Tron te ensenya el arte del Bash.\n");
...
void event_enter(object enterer, string message)
{
	...
	if (enterer->query_guild_name() != "guildtroll")
		return;
	...
	if (query_fighting()) { // no saludamos y advertimos al que entra
		tell_object(enterer, "Observas a Tron altamente excitada.\n");
		return;
	}
