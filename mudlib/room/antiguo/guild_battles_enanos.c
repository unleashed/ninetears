...
void setup()
{
  set_short("%^RED%^Gremio de Battle Ragers Enanos: Area de Entrenamiento%^RESET%^");
  set_long("%^RED%^Gremio de Battle Ragers Enanos: Area de Entrenamiento%^RESET%^\n\n"
...
//  add_exit("north", ROOM+"stair2.c", "corridor");
  add_exit("norte", "/room/entryroom", "door");
//  modify_exit("south", ({"function", "go_daggfight"}));
  modify_exit("norte", ({"function", "go_daggfight"}));
  set_guild("guerreros/battleenano");
	set_teaching_person("khalim");
}

void reset()
{
	if (!guildmaster) {
		guildmaster = clone_object("/baseobs/guildmasters/khalim.c");
...
int go_daggfight()
{
  if(this_player()->query_guild_name() == "battle rager enano" && guildmaster)
  {
	if (!guildmaster->query_fighting())
		tell_object(this_player(), "Khalim exclama: Sirva tu espada para tu causa!\n");
  }
  return 1;
}
