...
void setup()
{
  set_short("%^RED%^Gremio de Battle Ragers Duergar: Area de Entrenamiento%^RESET%^");
  set_long("%^RED%^Gremio de Battle Ragers Duergar: Area de Entrenamiento%^RESET%^\n\n"
...
//  add_exit("north", ROOM+"stair2.c", "corridor");
  add_exit("oeste", "/room/camino/caminito00", "door");
//  modify_exit("south", ({"function", "go_daggfight"}));
  modify_exit("oeste", ({"function", "go_daggfight"}));
  set_guild("guerreros/battleduergar");
	set_teaching_person("khuthul");
}

void reset()
{
	if (!guildmaster) {
		guildmaster = clone_object("/baseobs/guildmasters/khuthul.c");
...
int go_daggfight()
{
  if(this_player()->query_guild_name() == "battle rager duergar" && guildmaster)
  {
	if (!guildmaster->query_fighting())
		tell_object(this_player(), "Khuthul exclama: Sirva tu espada para extender el Mal!\n");
  }
  return 1;
}
