...
int banish(string str) {
  ...
  if(!this_player()->query_creator())
    return 0;
  notify_fail("Uso : banish <name> <reason>\n");
  if(!str)
    return 0;
  ...
  if(file_size("/banish/"+str+".o") >= 0) {
    notify_fail("Ese nombre ya esta baneado.\n");
    return 0;
  }
  if("secure/login"->test_user(str)) {
    notify_fail("Eso es un jugador.  Debes borrar o mover la ficha primero.\n");
    return 0;
  }
  ban = "Baneado por : "+(string)this_player()->query_name()+"\n";
  ban += ctime(time())+"\n";
  ban += "Razon del ban : " + str1 + "\n";
