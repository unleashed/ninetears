...
void setup() {
	...
	set_gp(1000);
      ...
      set_long("Khuthul, el guardian del gremio de los Battle Ragers Duergar.\n\n"
...
//               1, "@grin evilly $lname$",
	...
	set_guild("warriors/battleduergar");
...
int do_bash(string str) {
	...
	if (member_array("bash", TP->query_known_commands()) != -1) {
		notify_fail("Khuthul te dice: No puedo ensenyarte lo que ya sabes.\n");
		return 0;
	}
	if (TP->query_guild_name() != TO->query_guild_name()) {
		notify_fail("Khuthul te dice: Bash? No se de que me hablas.\n");
		return 0;
	}
...
void event_enter(object enterer, string message)
{
	...
	if (enterer->query_guild_name() != "battle rager duergar")
		return;
