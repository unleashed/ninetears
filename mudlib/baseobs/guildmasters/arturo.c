...
void setup() {
	...
	set_gp(1000);
      ...
      set_long("Arturo, el guardian del gremio de los guerreros.\n\n"
...
//               1, "@grin evilly $lname$",
	...
	set_guild("warrior");
	add_attack_spell(60, "go_berserk", ({"/d/gremios/comandos/go_berserk", "go_berserk",0,2}));
...
void event_enter(object enterer, string message)
{
	...
	if (enterer->query_guild_name() != "warrior")
		return;
	...
	if (query_fighting()) { // no saludamos y advertimos al que entra
		tell_object(enterer, "Observas a Arturo altamente excitado. %^BOLD%^%^RED%^Podria atacarte a ti inmerso en su furia!%^RESET%^\n");
		return;
	}
