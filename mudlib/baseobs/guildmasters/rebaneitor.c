...
void setup() {
      set_name("rebaneitor");
      set_short("rebaneitor");
      set_al(60);
      set_main_plural("rebaneitors");
	...
	set_gp(1000);
      ...
      set_long("Rebaneitor, el guardian del gremio de los Guerreros Elfos.\n\n"
		"Es un crack, yo de ti no me meteria con el.\n\n");
      adjust_money(random(300), "gold");
//      set_aggressive(0);
	new("/baseobs/weapons/tridente_negro")->move(this_object());
	new("/baseobs/weapons/tridente_negro")->move(this_object());
...
void event_enter(object enterer, string message)
{
	...
	if (enterer->query_guild_name() != "guerrero elfo")
		return;
