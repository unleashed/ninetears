...
void setup() {
      ...
      set_short("khalim");
      set_al(60);
      set_main_plural("khalims");
      add_alias("guildmaster");
      set_race("dwarf");
	set_language("common"); // si no me habla dwarf, aunke esta fn rula, esta demasiado orientada a player!
	set_gender(1);
  ...
  add_property("nodisarm",1);
      ...
      set_long("Khalim, el guardian del gremio de los Battle Ragers Enanos.\n\n"
		"Es un crack, yo de ti no me meteria con el.\n\n");
      adjust_money(random(300), "gold");
//      set_aggressive(0);
	new("/baseobs/weapons/twohandedaxe")->move(this_object());
	new("/baseobs/armours/platemail")->move(this_object());
...
//               1, "@grin evilly $lname$",
	...
	set_guild("warriors/battleenano");
	set_guild_ob("/d/gremios/guerreros/battleenano");
	add_attack_spell(60, "rabia", ({"/d/gremios/comandos/go_berserk", "go_berserk",1,0}));
	add_attack_spell(70, "bash", ({"/d/gremios/comandos/bash", "bash",0,0}));
...
int do_bash(string str) {
	...
	if (member_array("bash", TP->query_known_commands()) != -1) {
		notify_fail("Khalim te dice: No puedo ensenyarte lo que ya sabes.\n");
		return 0;
	}
	if (TP->query_guild_name() != TO->query_guild_name()) {
		notify_fail("Khalim te dice: Bash? No se de que me hablas.\n");
		return 0;
	}
	tell_object(TP, "Khalim te ensenya el arte del Bash.\n");
...
void event_enter(object enterer, string message)
{
	...
	if (enterer->query_guild_name() != "battle rager enano")
		return;
	...
	if (query_fighting()) { // no saludamos y advertimos al que entra
		tell_object(enterer, "Observas a Khalim altamente excitado. %^BOLD%^%^RED%^Podria atacarte a ti inmerso en su furia!%^RESET%^\n");
		return;
	}
