...
//      set_aggressive(0);
// armaduragnomica.c  baston_eog.c  capagnomica.c
	new("/baseobs/guildmasters/obj/aenima/baston_eog")->move(this_object());
	if(random(1))
	{
	new("/baseobs/guildmasters/obj/aenima/armaduragnomica")->move(this_object());
	}
	else
	{
        new("/baseobs/guildmasters/obj/aenima/capagnomica")->move(this_object());
	}
...
void event_enter(object enterer, string message)
{
	...
	if (enterer->query_guild_name() != "mago de agband")
		return;
	...
	command("say Saludos, "+ msg + " " + enterer->QCN + ".\n");
}
