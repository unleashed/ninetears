...
void setup() {
	poner_ayuda("\nNombre del Comando: Aplastar\n"
	    ...
	    "Descripcion: \n"
	    "Esta habilidad permite al Barbaro lanzar un poderoso ataque con "
	    "un arma a dos manos capaz de destrozar a los mas fuertes enemigos.\n");
 	gasta_energia(10);
...
int condiciones() {
	...
	if (!ob||ob->query_dead()) return notify_fail("Tu oponente ha muerto y no podra disfrutar del golpe aplastante.\n");
...
int efecto( string inutil, object masinutil ) {
	...
	mychance=me->query_level()+me->query_str()+me->query_habilidad("aplastar")/5;
	itschance=ob->query_level()+ob->query_dex();
	itschance*=random(100-ob->query_total_ac());
  	mychance*=random(275-me->query_thac0());
  	...
  	if(random(mychance)+random(me->query_habilidad("aplastar"))>random(itschance)) {
		...
		pupa=me->query_level()*4+me->query_str()*roll(1,5)+random(me->query_habilidad("aplastar"));
    		ob->adjust_hp(-pupa, me);
		if(ob->query_level()>me->query_level()) me->adjust_sp(1);
  		}
  	else {
