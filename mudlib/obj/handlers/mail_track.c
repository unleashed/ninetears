...
#define SECURITY if(!TP->query_creator()) return 0;
...
int add_mailer(object ob, string str) {
  	int i;
  	if ((i=member_array(str, our_mailers)) == -1) our_mailers += ({ ob, str });
  	else {
    		write(i);
...
object find_mailer(string str) {
  	...
  	if ((i=member_array(str, our_mailers))==-1) return 0;
...
int delete_mailer(object ob) {
  	...
  	if ((i=member_array(ob, our_mailers))==-1) return 0;
...
void save_me() { save_object("/save/mail_track",1); } /* save_me() */

void load_me() { restore_object("/save/mail_track",1); } /* load_me() */

string *query_mailing_lists() { return keys(mailing_lists) + get_dir("/d/"); } /* query_mailing_lists() */

/* does this list exist??? */
int query_list(string list) { return !undefinedp(mailing_lists[list]) || (file_size("/d/"+list) == -2); } /* query_list() */

string *query_members(string list) {
  	if (mailing_lists[list]) return mailing_lists[list][0] + ({ });
  	if (file_size("/d/"+list) == -2)return (string *)("/d/"+list+"/master")->query_members() + ({ (string)("/d/"+list+"/master")->query_lord() });
  	return ({ });
	} /* query_members() */

int query_member(string list, string name) {
  	if (!mailing_lists[list]) {
		if (file_size("/d/"+list) == -2) return (int)("/d/"+list+"/master")->query_member(name);
    		else return 0;
		}
...
int query_controller(string list, string name) {
  	if (!mailing_lists[list]) return 0;
...
int create_list(string list, string creator) {
    	SECURITY
  	if (mailing_lists[list]) return 0;
  	if (file_size("/d/"+list) == -2) return 0;
  	if ("/secure/login"->test_user(list)) return 0;
...
int add_controller(string list, string mem) {
  SECURITY
  if (!mailing_lists[list]) return 0;
  if (member_array(mem, mailing_lists[list][1]) != -1) return 0; /* already a member... */
