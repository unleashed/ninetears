...
int soul_command(string verb, string str, object m)
{
    ...
    if (liv)
    {
        ...
        if (!sizeof(ob))
        {
            notify_fail(liv+" no esta conectado.\n");
            return 0;
        }
        ...
        if (sizeof(ob) == 1)
        {
            ...
            {
                tell_object(me,ob[0]->query_cap_name()+" esta "
                  "bloqueando tus souls.\n");
                return 1;
            }
            ob[0]->event_soul(me,
	...
	// humm, mejor usar query_hidden() no?
    ...
    {
        if (!livfail)
            notify_fail("No tienes coraje para hacer eso.\n");
        else
            notify_fail("No puedo encontrar a '"+livfail+"'.\n");
        return 0;
    }
