...
int dispell()
{
tell_room(ENV(TO),"Gabriel traza un arco con su brazo y el aura que le rodeaba desaparece.\n");
...
{
tell_room(ENV(TO),"Gabriel pronuncia las palabras: baliis desaferum trus.\n");
llamado=1;
call_out("dispell",2);
}
 ::heart_beat(); 
...
setup()
  {
  set_name("gabriel");
  set_short("%^MAGENTA%^Gabriel el sonyador%^RESET%^");
  add_alias("bardo");
  set_main_plural("%^MAGENTA%^Bardos%^RESET%^");
  add_plural("bardos");
  add_adjective("sonyador");
  ...
  set_al(-1000);  
  set_thac0(-100);
  adjust_money(random(5)+30,"platinum");
...
"/global/do_chat"->do_channel("bardo",str,"Gabriel");           
...
int quitar()
{
   TO->remove_timed_property("nocast");
   return 1;
}
...
"/global/do_chat"->do_channel("bardo",str,"Gabriel");           
ob = TP;
write_file("/d/oficios/bardos/logs/gabriel.txt",""+TP->query_cap_name()+" "+ctime(time())+"\n");
return(::do_death());
}
