...
void setup()
{
  set_short("Gremio de %^ORANGE%^Brujos%^RESET%^: Sala de aprendizaje%^RESET%^");
...
//  add_exit("north", ROOM+"stair2.c", "corridor");
  add_exit("abajo", GREMIO+"piso1_1", "corridor");
  set_guild("other/testmago");

set_teaching_person("discordia");
}

void reset()
{
	if (!guildmaster) {
                guildmaster = clone_object(NPC+"discordia.c");
