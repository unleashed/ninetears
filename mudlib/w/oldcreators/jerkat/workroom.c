...
void setup()
{
   ::setup(); //Imprescindible en las rooms que son baules. added by leviathan.
   set_short("%^BOLD%^%^BLUE%^La Sala de las %^CYAN%^Mil %^RED%^Maravillas%^RESET%^");

   set_long("\n%^BOLD%^%^BLUE%^La Sala de las %^CYAN%^Mil %^RED%^Maravillas%^RESET%^\n\n");
   ...
   //so we use the function add_clone to bring them into your rooms.
	add_clone("/w/jerkat/valle_dragon/nopc/dummy");
   ...
   add_exit("tablon","/w/common","door");
  add_exit("centro","/room/entryroom.c","door");
  add_exit("valle","/w/jerkat/valle_dragon/room/valle_01","door");
  add_exit("trior","/w/jerkat/valle_dragon/room/trior/trior_entrada","door");
