...
int inactivo; // Tyrael
mixed equipo; //Vilat 31.08.2002

void set_creator(int i);
void save_equipo(); //Vilat 31.08.2002
void start_player();
void run_away();
...
void move_player_to_start(string bong, int newp, int going_invis) {
	...
	MEMORIAH->add_ip(query_ip_number(TO),capitalize(name),0);
    ...
    */
    ...
    if (level < 16)
    	set_short("%^BOLD%^GREEN%^"+CAP(name)+"%^RESET%^");
    else switch (CAP(query_race_name())) {
    	case "Drow":
    		set_short("%^BOLD%^BLACK%^"+CAP(name)+"%^RESET%^");
		break;
	case "Gnomo":
	case "Gnome":
	case "Halfling":
	...
	case "Enano":
	case "Dwarf":
		set_short("%^BOLD%^%^CYAN%^"+CAP(name)+"%^RESET%^");
		break;
	case "Humano":
	case "Human":
		if (query_guild_name() == "paladin")
			set_short("%^BOLD%^CYAN%^"+CAP(name)+"%^RESET%^");
		else if (query_guild_name() =="clerigo de jade")
			set_short("%^BOLD%^RED%^"+CAP(name)+"%^RESET%^");
		else set_short("%^BOLD%^"+CAP(name)+"%^RESET%^");
		break;
	default:
		set_short("%^BOLD%^RED%^"+CAP(name)+"%^RESET%^");
    }
...
void public_commands() {
	...
	// NI IDEA DE PQ COJONES EL STOP CON PARAR PONE Ein?
    add_action("stop","par*ar");
    add_action("invent","i*nventory");
    //add_action("invent","i");
...
int save_me() 
{
    ...
    if (query_property("guest")) {
	write("No se pueden salvar a los invitados...\n");
	return ;
    }
...
int really_quit()
{
	...
	MEMORIAH->remove_ip(query_ip_number(TO),capitalize(name),0);
...
string query_description()
{
	...
	if (TO->query_group_name() != "Sin clan.") {
		s += "Pertenece al Clan " + TO->query_group_name() +".\n";
	}
	if (TO->query_race_group_name() != "Sin casa.") {
...
string long(string str, int dark) {
    ...
    if (str == "sol" || str == "luna" || str == "tiempo"
	|| str == "sun" || str == "moon" || str == "weather") {
...
// Quark, adding heart beat to curses.
	...
	if (!net_dead) 
	{
	    say(query_cap_name()+" se vuelve palido, se empedrola y se "+
	      "convierte en una estatua.\n");
	    event(users(), "inform", query_cap_name() + " ha perdido " +
	      "su conexion", "link-death");
	    save_me();
	    this_object()->quit();
	    /* 
	    for (i=0;i<sizeof(attacker_list);i++)
	...
	// considerar poner aki un save y quit (dentro de else)
     /*  Idle CODE in pa ke Leviathan no me pille en dias a base de idle =) */
    } else {
	net_dead = 0;
	if (!inactivo) inactivo = MAX_IDLE*60;
	if (query_idle(this_object()) > inactivo)
	    if(my_file_name != "/global/god" && my_file_name != "/global/demi" &&
	      my_file_name != "/global/lord" && my_file_name != "/global/creator")
	    {
		say(query_cap_name()+" ha estado inactivo demasiado tiempo, "+query_pronoun()+
		  " se desvanece en una nube de aburrimiento.\n");
		write("Has estado inactivo demasiado tiempo. Aus!\n");
		quit();
		return ;
	    }
	    else if (my_file_name == "/global/creator") {
		if (query_idle(TO) > inactivo * 2) {
	    ...
	    {
	    	if (query_idle(TO) > inactivo * 3) {
	    		tell_object(TO, "Has estado inactivo a sako, aus!\n");
	    		quit();
	    		return;
	    	}
	    	else if(query_idle(TO) > inactivo * 2) {
	    		if(TO->query_invis() < 2) {
		    ...
		    tell_object(this_object(),"Has estado inactivo demasiado tiempo, poniendote en"
		      " invis 1.\n");
	...
	// Supongo que esto se hace siempre
...
if(hp_counter >= 4 && "/global/omiq.c"->omiq_in_progress())
{
    ...
    */
	...
	{
		// todo esto taria mejor con switch
		if (query_race() != "orco") {
	    heal_hp(1);
	    heal_gp(1, intox);
	    hp_counter = 0;
		}
		else { // orcos regeneran antes y mas
			heal_hp(2);
			heal_gp(2, intox);
			hp_counter = 5;
		}
...
void display_monitor(int i)
{
    ...
    if(!monitor) return;
    if(i) COLOR = "%^GREEN%^";
    tell_object(this_object(),COLOR+sprintf("Pv: %d/%d Pg: %d/%d\n",hp,max_hp,gp,max_gp)+
      "%^RESET%^\n");
    return;
}
...
int monitor(string str) 
{
	...
	notify_fail("Sintaxis: monitor <on/off>\n");
	return 0;
    }
    if (monitor)
	write("Tu monitor de vida esta activo.\n");
    else
	write("Tu monitor de vida esta inactivo.\n");
    return 1;
}
...
int query_hidden()
{
    ...
    // fix de performance ;) - Tyrael - May'02
