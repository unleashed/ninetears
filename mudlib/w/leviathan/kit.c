...
void setup()
{
   ...
   set_name("Anillo de Keltia");
   add_alias("kit");
   add_alias("anillo de keltia");
   reset_drop(); // Impide que se pueda dejar en el suelo y que se caiga al 
  // set_wearable(2);
   set_short("%^BOLD%^%^WHITE%^Anillo de %^BLACK%^Keltia%^RESET%^");
   set_long("%^BOLD%^%^WHITE%^   Anillo de %^BLACK%^Keltia:%^RESET%^\n\n"
	" Es un objeto de poder, con el que su poseedor tiene un control absoluto sobre los"
	"reinos. Para mas informacion pon 'help anillo'.\n\n");
...
// Ni warra, sospecho que al ponertelo o equipartelo o algo, cuando entras 
// Te reconoce si eres un miembro del dominio Driade, y te saluda, eso 
// kit del dominio Driade, y que eres miembro del mismo.
// El free_action te permite moverte bien en sitios con agua, y poder hablar 

void badge_hi()
{
	// NO usable por mortales. - Tyrael - May'02
	if (ETO && interactive(ETO) && !ETO->query_creator()) {
		tell_object(ETO, "Que haces tu con un Kit?!?\n");
		call_out("dest_me", 0);
		return;
	}
/*   if( ETO && interactive(ETO) &&
   ...
   {
      write("%^CYAN%^\n\nLos espiritus de %^BOLD%^Driade %^RESET%^%^CYAN%^te 
dan la bienvenida mi Lord.%^RESET%^\n\n");
      ETO->add_timed_property("free_action",1,3000000);
   }
      write("\n%^BOLD%^%^GREEN%^Tu kit de comunicaciones es el 
correcto.%^RESET%^\n\n");
}
*/
...
// Fale.. esto talmente parece que comprueba si eres un miembro del dominio 
...
int do_clonada()
{
        write("\n%^BOLD%^%^RED%^Aqui deberian salir las clonadas o posibles 
clonadas.\n  ");
        write(read_file("/log/ENTER_SOSPECHOSO"));
	return 1;
}
...
// Para las peleas actuales de todos los pj y npcs que se encuentran en la 
...
int do_peace()
{
    ...
    tell_room(environment(this_player()), this_player()->query_cap_name()+
              " alza sus manos al cielo y grita: %^YELLOW%^DEJAD DE LUCHAR 
MORTALES !!%^RESET%^\n", ({this_player()}));
    return 1;
}
...
// A quien le importa la hora que es en sabediosdonde? no se si se refiere 
...
int do_times()
{
    ...
    write("Son las "+ctime(time() - 28800)+" por la zona de las 
montanyas.\n");
...
//Esto compueba las salidas de una room, te dice a que sitio apuntan, si la 
...
int exit()
{
  ...
  for(i=0;i<sizeof(dirs);i=i+2)
  {
    ...
    if(file_size(dirs[i+1]) == -1)
      write("El fichero "+adjust_string_size(dirs[i+1],42)+"%^BOLD%^RED%^ no 
existe.%^RESET%^\n");
    else
    {
      string ppp;

	catch(dirs[i+1]->dest_me()); // dies
...
"========================================================================\n"
         "                 %^GREEN%^ CREADORES DE 
%^CYAN%^"+capitalize(str)+"%^GREEN%^ CONECTADOS AHORA%^WHITE%^               
    \n"
