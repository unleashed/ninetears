...
void setup()
{
	...
	set_name("helada");
	set_short("Navaja de %^BOLD%^BLACK%^Albacete%^RESET%^");
	set_main_plural("Navajas de %^BOLD%^BLACK%^Albacete%^RESET%^");
	set_long("Una impresionante navaja de albacete que arranca las almas de sus enemigos.\n");
	add_alias(({"navaja","albacete"}));
	add_plural("navajas");
	set_enchant(1);
	reset_drop();
}
