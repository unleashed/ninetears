...
int open_chest() 
{
   if(!open)
   {
      ...
      write("\n   El "+this_object()->query_name()+" está"
            " cerrado con llave.\n\n");
      return(1);
   }
   write("El cofre ya está abierto.\n");
   return(1);
}
int close_chest()
{
...
int box_unlock(string locked_ob)
{
   ...
   write("\n   Abres el "+locked_ob+".\n\n");
     say("\n   "+this_player()->query_cap_name()+" sonrie "
         " mientras "+this_player()->query_pronoun()+" abre el "
         " "+locked_ob+".\n\n");
   return(1);
}
...
void init()
{
add_action("open_chest","abrir");
add_action("close_chest","cerrar");
  ::init();
}
