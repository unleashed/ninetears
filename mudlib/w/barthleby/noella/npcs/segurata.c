...
void event_enter(object ob, string mensaje, object donde)
{
   if ((int)ob->query_timed_property("guardias_noella"))
   {
      ...
      else
      {
         ...
         do_command("say Fuera de aquí, delincuente!!!!!!.\n");
      }
   }
...
{
   do_command("'Aquí no queremos peleas callejeras, fuera!");
   ob1->run_away();
   ob2->run_away();
}
...
void do_steal(string quien)
{
   ...
   if (this_player()->query_cha()<random(21))
   {
      if (random(4)==random(4))
      {
         ...
         do_command("say Fuera de aquí, delincuente!!!!!!.\n");
         this_player()->add_timed_property("guardias_noella", 1, 300);
      }
      else
      {
         write(this_object()->query_cap_name()+" se fija en ti, parece "
               "que ha estado a punto de pillarte.\n");
         do_command("say Oye, tu, más vale que no te pille haciendo nada "
                    "extraño.");
      }
   }
   return 0;
...
void setup()
{
   ...
   set_long("Es un guardia de seguridad de Noella. Parece fuerte y dispuesto "
   ...
   add_clone(ARMA+"bastard_sword",2);
   add_clone(ARMADURA+"cota_de_malla",1);
   ::init_equip();
