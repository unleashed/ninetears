...
void create() {
  ...
  set_short("Taberna");
  set_long(
    "Estas en una taberna. Puedes comprar bebidas o comidas.\n"
    "Hay un menu que puedes leer.\n");
  set_light(60);
  menu_header = "Puedes leer en el Menu:";
  display_headings = 1;
  ::create();
  ...
  //set_open_condition(1);
}
 
void init() {
  ::init();
  add_action("buy", "buy");
  add_action("buy", "compra*r");
  add_action("buy", "come*r");
  add_action("do_read","read");
  add_action("do_read","lee*r");
...
void add_menu_items(mixed *bebidas) {
  int loop;
  for(loop=0; loop<sizeof(bebidas); loop++) {
    if(sizeof(bebidas[loop]) == 5) {
      mixed *dr;
...
int do_read(string str)
{
  int temp;
   ...
   if(!test_open()){
      write("En este momento por motivos dispares no te pueden atender.\n");
      return 0;
   }
   if(str && lower_case(str) == "me" || TP->query_player() == 1 )
   {
  ...
  if((temp = do_race_adjust(temp,this_player()->query_race()))==
         -101) {
                write("Este establecimiento rehusa a servir a los tu raza.\n");
                return 1;
  }
      write(read());
      return(1);
...
// I hope cost didn't need to be mixed :)   Anirudh
  ...
  if (test_open() != 1)
  {
      if(test_open() == -1)
      {
        write("Aqui no se sirve comida a los de tu raza.\n");
        return 1;
      }
      write("En este momento por motivos dispares no te pueden atender.\n");
      return 1;
...
return 0;
  ...
  if(!sizeof(list))
  {
     notify_fail("Comprar <comida/bebida> [para jugador]");
     return 0;
  }
  forplay = member_array("para",list);
  if(forplay > 0)
  {
      ...
      notify_fail("Esta persona no se encuentra aqui.\n");
   forwho = present(list[forplay+1]);
      ...
      if(!forwho->query_consent("bebidas"))
      {
        write((string)forwho->query_cap_name()+" rechaza tu ofrecimiento.\n");
        tell_object(forwho,this_player()->query_cap_name()+" prueba a "
             "comprarte algo, pero tu lo rechazas.\n");
        say((string)this_player()->query_cap_name()+" ofrece comprarle a "
             +forwho->query_cap_name()+" algo.\n",({ this_player(),forwho}) );
      return 1;
      }
      str = "";
      for(i = 0;i < forplay;i++)
  ...
  if(!menu_items[str]) 
   {
    notify_fail("Lo siento, "+str+" no esta en el menu.\n");
    return 0;
  }
  ...
  if((cost = do_race_adjust(cost,this_player()->query_race()))==
          -101) { notify_fail("Este establecimiento no sirve a personas "
                     "de tu raza.\n");
                  return 0;
  }
  if(this_player()->query_value() < cost) {
    notify_fail("Eres demasiado pobre para esto.\n");
    return 0;
  }
#ifdef AUTOMATIC
  ...
  forwho->adjust_intox(intox);
   ...
   {
      tell_room(this_object(),(string)this_player()->query_cap_name()+" compra "+
      (string)forwho->query_cap_name()+" "+str+".\n",({ }));
   }
  tell_object(forwho,"Te "+drmess+".\n");
  ...
  if(error) {
    write("No puedes cargar con algo asi.\n");
    food->dest_me();
    return 1;
  }
  this_player()->pay_money( ({"copper",cost }) );
#endif
  // ISHMAR, POR SEGUNDA VEZ, Y LO KE JODE
