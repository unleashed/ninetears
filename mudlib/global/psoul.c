...
int action_check(string str) {
    	if (sizeof(in_command) && in_command[0] == str) return 0;
    	if(this_player(1)) if(TP->query_property("_cre")) log_file("."+this_player()->query_name(), str+"\n");
    	if(this_object()->query_property(PASSED_OUT_PROP)) {
		mixed p_o_p = this_object()->query_property(PASSED_OUT_PROP);
		if (str == "quit") return 0;
		if ( stringp(p_o_p) ) {
	    		write(p_o_p);
	    		if(!this_object()->query_lord()) return 1;
			}
		else {
	    		write("Estas incosciente, no puedes hacer nada por el momento.\n");
	    		if(!this_object()->query_lord()) return 1;
			}
    		}
    	if (str == "stop" || str == "restart" || str=="parar") {
		if (!sizeof(queued_commands)) return 1;
		queued_commands = ({ });
		if (interupt && time_left < 0) catch(call_other(interupt[1], interupt[0], -time_left, interupt[2]));
		write("Comandos en cola borrados.\n");
		time_left = 0;
		return 1;
    		}
    	if (time_left < 0 || (!doing_it && sizeof(queued_commands))) {
		queued_commands = queued_commands[0..queue_depth[0]-1]+ ({ str })+queued_commands[queue_depth[0]..10000];
		queue_depth[0]++;
		return 1;
    		}
    	if (interupt) catch(call_other(interupt[1], interupt[0], 0, interupt[2]));
    	interupt = 0;
    	...
    	if (!sizeof(queue_depth)) queue_depth = ({ 0, 0 });
    	else queue_depth = ({ queue_depth[0] }) + queue_depth;
//    	command(str);
...
int lower_check(string str) {
    	time_left += DEFAULT_TIME;
    	return !(str == "stop" || str == "restart" || str=="parar");
	}
